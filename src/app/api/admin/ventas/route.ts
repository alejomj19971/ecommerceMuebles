...
import { generarNumeroOrdenVenta } from "@/lib/numero-orden";
...
export async function POST(request: Request) {
  ...
  const venta = await prisma.$transaction(async (tx) => {
    const numeroOrden = await generarNumeroOrdenVenta(tx);
  ...
  return NextResponse.json({ ok: true, id: venta.id.toString() });
}
